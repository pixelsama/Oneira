...
#[tauri::command]
pub async fn generate_image(app: AppHandle, payload: GeneratePayload) -> Result<Vec<String>, String> {
    // 1. Get Settings
    ...
    if let Some(data) = json.get("data").and_then(|d| d.as_array()) {
        for (i, item) in data.iter().enumerate() {
            if let Some(image_url) = item.get("url").and_then(|v| v.as_str()) {
                ...
                // Download image
                let img_bytes = client.get(image_url)
                    .send()
                    .await
...
async fn generate_openai(
    ...
    let api_url = store.get("api_url").and_then(|v| v.as_str().map(|s| s.to_string()))
